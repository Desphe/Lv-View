...
class CurrencyTable1 extends PureComponent {
  ...
  getAllButton(btnConfig) {
    return btnConfig.map((todo,i) => (
      <Button key={"btn_"+todo.funCode} icon={todo.icon} type={todo.type} onClick={() => this.handleModalVisible(true)}>
        {todo.btnName}
      </Button>)
    )
  }
  ...
  render() {
    ...
    const { columns,btnConfig } = data;
    const { selectedRows, modalVisible, updateModalVisible, viewVisible } = this.state;
